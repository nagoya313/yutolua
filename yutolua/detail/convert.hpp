#ifndef YUTOLUA_DETAIL_CONVERT_HPP_
#define YUTOLUA_DETAIL_CONVERT_HPP_
#include <array>
#include <string>
#include <type_traits>
#include <vector>
...
bool convert(lua_State *lua, int index) {
  type_check<LUA_TBOOLEAN>(lua_type(lua, index));
...
T convert(lua_State *lua, int index) {
  type_check<LUA_TNUMBER>(lua_type(lua, index));
...
std::string convert(lua_State *lua, int index) {
  type_check<LUA_TSTRING>(lua_type(lua, index));
